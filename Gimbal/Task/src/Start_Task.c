...
#define VOFA_TASK_PRIO 24
#define VOFA_TASK_STK_SIZE 128
...
void start_task(void *pvParameters)
{
    ...
    //    // VOFA调试
    //    xTaskCreate((TaskFunction_t)VOFATask,                // 任务函数
    //                (const char *)"VOFATask",                // 任务名称
    //                (uint16_t)VOFA_TASK_STK_SIZE,            // 任务堆栈大小
    //                (void *)NULL,                            // 传递给任务函数的参数
    //                (UBaseType_t)VOFA_TASK_PRIO,             // 任务优先级
    //                (TaskHandle_t *)&User_Tasks[VOFA_TASK]); // 任务句柄
