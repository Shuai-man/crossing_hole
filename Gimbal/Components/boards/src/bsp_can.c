...
void DataReceive(CAN_HandleTypeDef *hcan, CAN_RxHeaderTypeDef *rx_header, uint8_t *data)
{
	...
	else if (hcan == ENCODER_CAN && rx_header->StdId == ENCODER_ID)
	{
		...
		if (rx_header->DLC >= 7U && data[0] == 0x07U && data[1] == 0x01U && data[2] == 0x01U)
		{
			memcpy(&encoder.frame, data, 7);
			encoder.value = ((uint32_t)encoder.frame.data4 << 24) |
				((uint32_t)encoder.frame.data3 << 16) |
				((uint32_t)encoder.frame.data2 << 8) |
				(uint32_t)encoder.frame.data1;
