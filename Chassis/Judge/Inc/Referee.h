...
/* Defines -------------------------------------------------------------------*/
#define Referee_UART huart4
#define Referee_IRQHandler USART4_IRQHandler
...
/* 0x020X --------------------------------------------------------------------*/
typedef __packed struct  //0x0201 机器人性能体系
{
	uint8_t robot_id;
	uint8_t robot_level;
	uint16_t current_HP; 
...
typedef __packed struct  //0x0202 实时底盘功率和枪口热量
{
	uint16_t reserved;
	uint16_t reserved_1;
	float    reserved_2;
...
typedef __packed struct  //0x20C 雷达标记进度
{
	uint8_t mark_progress;
} radar_mark_data_t;

typedef __packed struct //0x20D 哨兵自主决策信息同步
{
...
typedef __packed struct  //0x0302 自定义控制器与机器人交互数据
{
	uint8_t *data;
} custom_robot_data_t;

typedef __packed struct  //0x0303 选手端小地图交互数据
{
...
typedef __packed struct  //0x0305 选手端小地图接收雷达数据
{
...
typedef __packed struct  //0x0306 自定义控制器与选手端交互数据
{
	uint16_t key_value;
...
typedef __packed struct  //0x0307 选手端小地图接收哨兵数据
{
	uint8_t intention;
...
typedef __packed struct  //0x308 选手端小地图接收机器人数据
{ 
	uint16_t sender_id;
	uint16_t receiver_id;
...
typedef __packed struct  //0x309 自定义控制器接受机器人数据
{ 
	uint8_t *data;
} robot_custom_data_t;
...
extern dart_info_t                     Dart_Remaining_Time;
...
extern buff_t                   Buff;
extern hurt_data_t              Hurt;
extern shoot_data_t             Shoot;
extern projectile_allowance_t   Projectile_Allowance;
extern rfid_status_t            RFID_Status;
extern dart_client_cmd_t        Dart_Client_Cmd;   //20A
extern ground_robot_position_t  Ground_Robot_Position;  //20B
extern radar_mark_data_t        Radar_Mark_Data;
extern sentry_info_t            Sentry_Info;
extern radar_info_t             Radar_Info;

/* 0x030X */
extern robot_interaction_data_t      Robot_Interaction_Header_Data; //301
extern custom_robot_data_t           Custom_Robot_Data;          //302
extern map_command_t                 Map_Command;                   //303   
extern remote_control_t              Remote_Control;              //305  
extern map_robot_data_t              Map_Robot_Data;                  //304 
extern custom_client_data_t          Custom_Client_Data;							//306
extern map_data_t                    Map_Data;                 //307
