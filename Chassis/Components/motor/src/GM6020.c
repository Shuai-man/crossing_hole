 ...
 * @param[in] control_current 控制电流,或电压
 */
void GM6020_SendPack(uint8_t *can_data, uint32_t std_id, uint8_t motor_id, int16_t control, GM_6020_SEND_TYPE type)
{
    if (type == GM6020_CUR_MODE)
    {
        control = LIMIT_MAX_MIN(control, GM6020_MAX_CURRENT, GM6020_MIN_CURRENT);
    }
    else
    {
        control = LIMIT_MAX_MIN(control, GM6020_MAX_VOLTAGE, GM6020_MIN_VOLTAGE);
    }

    if (std_id == GM6020_STD_VOL_ID_1_4 || std_id == GM6020_STD_CUR_ID_1_4)
    {
        if (motor_id >= GM6020_MOTOR_ID_1 && motor_id <= GM6020_MOTOR_ID_4)
        {
            int index = (motor_id - GM6020_MOTOR_ID_1) * 2;
            can_data[index] = (int8_t)((control & 0xFF00) >> 8);
            can_data[index + 1] = (int8_t)(control & 0x00FF);
        }
    }
    else if (std_id == GM6020_STD_VOL_ID_5_7 || std_id == GM6020_STD_CUR_ID_5_7)
    {
