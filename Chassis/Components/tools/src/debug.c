...
void LossUpdate(Loss_Debugger *loss_debugger, float thresh_t)
{
    ...
    loss_debugger->can_dt = DWT_GetDeltaT(&loss_debugger->last_can_cnt);
		loss_debugger->loss_time = 0;
...
void LossJudge(Loss_Debugger *loss_debugger)
{
		loss_debugger->loss_time++;
}
