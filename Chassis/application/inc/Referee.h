...
typedef struct // 0x0202 实时功率热量数据
{
	...
	uint16_t shooter_id1_17mm_cooling_heat; //第一个17mm发射机构的枪口热量
...
typedef struct // 0x0204 机器人增益
{
	...
	uint8_t remaining_energy; //剩余能量状态
} ext_buff_musk_t;
...
typedef struct
{
	...
	uint8_t Enemy_Sentry_shootable : 1; // �з��ڱ��Ƿ��޵�
...
typedef struct
{
	uint16_t x; // ����ϵͳ���Ļ���������(��float ӳ�䵽 uint16_t : float*100 -> uint16_t)
	uint16_t y;
...
//#define NMD sizeof(graphic_data_struct_t)
