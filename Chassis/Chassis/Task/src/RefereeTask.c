...
void drawCapBar(graphic_data_struct_t *Graphic, uint8_t GraphOperate)
{
	...
	UI_Draw_Line(Graphic, "310", GraphOperate, 4, COLOR, CAP_BAR_WIDTH, CAP_BAR_UI_START_X, CAP_BAR_UI_START_Y, CAP_BAR_UI_START_X + (uint16_t)(cap_controller.cap_energy_pecent * CAP_BAR_LENGTH), CAP_BAR_UI_START_Y);
}
/**
 * @brief 锟斤拷锟斤拷系统锟斤拷锟斤拷锟斤拷锟斤拷
 * @param[in] void
 */
float DT;
//uint32_t T;
...
//			HAL_UART_Receive_DMA(&huart4, rx_buffer, sizeof(rx_buffer));
		...
		else if(UI_PushUp_Counter_500 == 53)
		{
			...
			UI_PushUp_Graphs(2, &referee_data.UI_Graph2, referee_data.Game_Robot_State.robot_id);	
			 // 在矩形上方绘制字符串
			 UI_Draw_String(&referee_data.UI_String.String, "004", UI_Graph_Add, 2, UI_Color_White, 20, 10, 3, CHASSIS_POS_UI_START_X, CHASSIS_POS_UI_START_Y - CHASSIS_POS_WIDTH / 2 - 200, "          ");
			 UI_PushUp_String(&referee_data.UI_String, referee_data.Game_Robot_State.robot_id);
		 
		}
		else if(UI_PushUp_Counter_500 == 131)
		{
			UI_Draw_String(&referee_data.UI_String.String, "006", UI_Graph_Add, 2, UI_Color_Orange, 20, 6, 3, 1280, 700, "MODE:0");
			UI_PushUp_String(&referee_data.UI_String, referee_data.Game_Robot_State.robot_id);
		}
...
//		}
		...
		{
			UI_Draw_String(&referee_data.UI_String.String, "105", UI_Graph_Add, 2, UI_Color_Green, 15, 11, 3, 1200, 70, "FRI_SPEED:");
			UI_PushUp_String(&referee_data.UI_String, referee_data.Game_Robot_State.robot_id);
		}
...
//		}
		...
		{
			//显示摩擦轮转速
			UI_Draw_Float(&referee_data.UI_Graph5.Graphic[0], "106", UI_Graph_Add, 3, UI_Color_Orange, 15, 2, 3, 1200, 40, infantry.friction_speed);
...
//		}
		...
		{
			//显示摩擦轮转速
			UI_Draw_Float(&referee_data.UI_Graph5.Graphic[0], "106", UI_Graph_Change, 3, UI_Color_Orange, 15, 2, 3, 1200, 40, infantry.friction_speed);
			UI_PushUp_Graphs(5, &referee_data.UI_Graph5, referee_data.Game_Robot_State.robot_id);
		}
		//更新频率为100毫秒
		else if(UI_PushUp_Counter_10 == 5)
		{
			if(gimbal_receiver_pack1.control_mode_action == CV_ROTATE)//小陀螺旋转
			{
				UI_Draw_String(&referee_data.UI_String.String, "004", UI_Graph_Change, 2, UI_Color_Orange, 20, 10, 3, CHASSIS_POS_UI_START_X, CHASSIS_POS_UI_START_Y - CHASSIS_POS_WIDTH / 2 - 200, "CV_ROTA:ON");
			}
			else{
				UI_Draw_String(&referee_data.UI_String.String, "004", UI_Graph_Change, 2, UI_Color_Orange, 20, 10, 3, CHASSIS_POS_UI_START_X, CHASSIS_POS_UI_START_Y - CHASSIS_POS_WIDTH / 2 - 200, "          ");
			}
			UI_PushUp_String(&referee_data.UI_String, referee_data.Game_Robot_State.robot_id);
		}
		else if(UI_PushUp_Counter_10 == 7)
		{
			if(gimbal_receiver_pack2.shot_aim_mode == 0)//
			{
				//不使用辅瞄
				UI_Draw_String(&referee_data.UI_String.String, "006", UI_Graph_Change, 2, UI_Color_Orange, 20, 6, 3, 1280, 700, "MODE:0");
			}else if(gimbal_receiver_pack2.shot_aim_mode == 1)
			{
				//普通辅瞄
				UI_Draw_String(&referee_data.UI_String.String, "006", UI_Graph_Change, 2, UI_Color_Orange, 20, 6, 3, 1280, 700, "MODE:1");
			}else if(gimbal_receiver_pack2.shot_aim_mode == 2)
			{
				//小符
				UI_Draw_String(&referee_data.UI_String.String, "006", UI_Graph_Change, 2, UI_Color_Orange, 20, 6, 3, 1280, 700, "MODE:2");
			}else if(gimbal_receiver_pack2.shot_aim_mode == 3)
			{
				//定点小陀螺
				UI_Draw_String(&referee_data.UI_String.String, "006", UI_Graph_Change, 2, UI_Color_Orange, 20, 6, 3, 1280, 700, "MODE:3");
			}else if(gimbal_receiver_pack2.shot_aim_mode == 4)
			{
				//大符
				UI_Draw_String(&referee_data.UI_String.String, "006", UI_Graph_Change, 2, UI_Color_Orange, 20, 6, 3, 1280, 700, "MODE:4");
			}
			UI_PushUp_String(&referee_data.UI_String, referee_data.Game_Robot_State.robot_id);
		}
