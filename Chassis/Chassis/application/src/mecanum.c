...
{
    // 转向PID初始化
    PID_Init(&infantry.turn_pid, 80, 0, 0.0, 20, 0, 0.0, 0, 0, 0.000, 0.000, 1, NONE);
...
void mecanum_follow_control()
{
    ...
    if(gimbal_receiver_pack2.shot_aim_mode == 2 || gimbal_receiver_pack2.shot_aim_mode == 4)
	{
		...
		if(fabs(infantry.error_angle) < 0.1)
		{
			infantry.set_yaw_v = 0;
		}
	}
