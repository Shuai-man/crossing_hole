...
void RemoteReceive(volatile unsigned char rx_buffer[])
{
    ...
    remote_controller.dji_remote.rc.poke = (rx_buffer[17] << 8) | rx_buffer[16];
...
{
    remote_controller.fly_state = Fly_State;
}

void setChassisFormat(enum CHASSIS_FORMAT chassis_format)
{
    remote_controller.chassis_format = chassis_format;
}

void setBombBayState(enum BOMB_BAY_STATE bay_state)
{
    remote_controller.bomb_bay = bay_state;
}
