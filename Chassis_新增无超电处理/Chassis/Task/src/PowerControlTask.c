...
void PowerControlTask(void const * argument)
{
	...
	while (1)
	{
		...
		{
			referee_power = referee_data.Game_Robot_State.chassis_power_limit;
		}else
		{
			referee_power = LIMIT_MAX_MIN(referee_data.Game_Robot_State.chassis_power_limit, 300, 40);
		}

		if (remote_controller.fly_state == IS_FLY)//飞坡
		{
			NingCapControl(referee_data.Power_Heat_Data.buffer_energy, referee_power, 350.0f);//全向轮300W飞坡姿态良好
		}
...
//			NingCapControl(referee_data.Power_Heat_Data.buffer_energy, 80.0f, 80.0f);
		}
